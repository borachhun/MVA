...
class Reflection_x(object):
    def __call__(self, pointcloud):
        reflected_pointcloud = np.array(pointcloud)
        reflected_pointcloud[:,0] *= -1
        return reflected_pointcloud
...
def default_transforms():
    return transforms.Compose([Reflection_x(),RandomRotation_z(),RandomNoise(),ToTensor()])
...
class MLP(nn.Module):
    ...
    def forward(self, input):
        ...
        x = F.relu(self.norm2(self.dropout(self.layer2(x))))
        x = self.layer3(x)
        return x
...
class PointNetFull(nn.Module):
    ...
    def forward(self, input):
        # YOUR CODE
        tnet3x3 = self.tnet(input).reshape(-1,3,3) + torch.eye(3).to(device)
        x = tnet3x3 @ input
        x = self.mlp1(x)
        x = self.mlp2(x)
        x = self.pool(x).flatten(start_dim=1)
...
if __name__ == '__main__':
    ...
    train(model, device, train_loader, test_loader, epochs = 100)
