...
def best_rigid_transform(data, ref):
    ...
    barycenter_ref = np.mean(ref, axis=1).reshape(-1,1)
    barycenter_data = np.mean(data, axis=1).reshape(-1,1)

    centered_ref = ref - barycenter_ref
    centered_data = data - barycenter_data
...
def icp_point_to_point(data, ref, max_iter, RMS_threshold):
    ...
    R_list.append(np.eye(data.shape[0]))
    T_list.append(np.zeros((data.shape[0], 1)))

    tree = KDTree(ref.T)
    for _ in range(max_iter):
        neighbor_index = tree.query(data_aligned.T, return_distance=False).squeeze()
        R, T = best_rigid_transform(data_aligned, ref[:, neighbor_index])

        data_aligned = R.dot(data_aligned) + T
        distances = np.sum(np.power(data_aligned - ref[:, neighbor_index], 2), axis=0)
        RMS = np.sqrt(np.mean(distances))

        # Convert the transformation from step transformation to with respect to initial data
        T = R @ T_list[-1] + T
        R = R @ R_list[-1]

        R_list.append(R)
        T_list.append(T)
        neighbors_list.append(neighbor_index)
        RMS_list.append(RMS)
